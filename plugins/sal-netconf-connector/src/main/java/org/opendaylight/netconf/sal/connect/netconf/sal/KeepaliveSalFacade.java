...
public final class KeepaliveSalFacade implements RemoteDeviceHandler {
    ...
    final class KeepaliveTask implements Runnable, FutureCallback<DOMRpcResult> {
        ...
        synchronized void disableKeepalive() {
            ...
            suppressed = true;
            suppressedCounter++;
            LOG.error("{}: KeepaliveTask.disableKeepalive, setting suppressed=true suppressedCounter={}", id,
                    suppressedCounter);
        }
        ...
        synchronized void enableKeepalive() {
            recordActivity();
            suppressedCounter--;
            LOG.error("{}: KeepaliveTask.enableKeepalive, suppressed is {} suppressedCounter={}", id, suppressed,
                    suppressedCounter);
            if (suppressedCounter > 0) {
                LOG.error("{}: Skipping to enable keepalive while expecting {} RPC reply", id, suppressedCounter);
                return;
            }
            if (suppressed) {
